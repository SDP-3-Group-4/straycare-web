...
export class PostsService {
  ...
  async getPosts(tab?: string, userId?: string, lat?: number, lng?: number) {
    ...
    if (tab === 'nearby') {
      ...
      if (userId && (referenceLat === undefined || referenceLng === undefined)) {
      ...
      if (referenceLat !== undefined && referenceLng !== undefined) {
        ...
        posts = posts.filter(post => {
          ...
          const dist = getDistance(referenceLat!, referenceLng!, post.latitude, post.longitude);
  ...
  async updatePost(id: string, authorId: string, data: any) {
    ...
    if (post.authorId !== authorId) throw new NotFoundException('Unauthorized to edit this post');
  ...
  async donateToPost(id: string, userId: string, amount: number) {
    ...
    if (post.authorId !== userId) {
      const donor = await this.prisma.user.findUnique({ where: { id: userId } });
