...
import { FileText, ShieldAlert, HeartHandshake, HelpCircle, X, Check } from 'lucide-react';
      ...
      <div className="relative bg-white rounded-3xl w-full max-w-lg overflow-hidden flex flex-col border border-[var(--sc-border)] shadow-2xl animate-in zoom-in-95 duration-200 max-h-[85vh]">
        ...
        <div className="flex items-center justify-between p-4 sm:p-5 border-b border-[var(--sc-border)] bg-gray-50/70">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <span className="text-[10px] font-bold text-[var(--sc-brand-600)] uppercase tracking-wider block">{data.badge}</span>
              <h2 className="text-base font-bold text-[var(--sc-text-primary)] truncate">{data.title}</h2>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-5 sm:p-6 overflow-y-auto flex flex-col gap-4">
          {data.content.map((sec, idx) => (
            <div key={idx} className="p-4 bg-gray-50/80 rounded-2xl border border-[var(--sc-border)] flex flex-col gap-1.5">
              <h4 className="font-bold text-sm text-[var(--sc-text-primary)]">{sec.heading}</h4>
