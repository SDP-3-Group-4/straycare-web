  ...
  return (
    <>
      <NavLink
        to="/profile"
        className={({ isActive }) =>
          `flex items-center justify-between p-4 rounded-2xl transition-all w-full text-left mt-2 group relative overflow-hidden ${
            isActive
      ...
      >
        <div className="flex items-center gap-3 relative z-10">
          <div className="flex-shrink-0 relative">
            <div
              className={`w-10 h-10 rounded-full overflow-hidden flex items-center justify-center bg-white/10 ${
                user.isVet || user.verifiedStatus
                  ? "ring-2 ring-white ring-offset-2 ring-offset-[var(--sc-brand-500)]"
          ...
          <div className="hidden xl:flex flex-col">
            <div className="flex items-center gap-1.5">
              <span
                className="font-bold text-white text-[15px] truncate max-w-[110px] notranslate"
                translate="no"
              >
                {user.displayName || "User"}
              </span>
              {(user.isVet || user.verifiedStatus) && (
                <span className="px-1.5 py-0.5 bg-white/20 text-white text-[9px] font-bold rounded-md ">
                  Vet
                </span>
              )}
            </div>
