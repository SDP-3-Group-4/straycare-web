  ...
  useEffect(() => {
    if (user?.uid) {
      fetchNotifications(user.uid)
        ...
        .catch((err) => console.error("Failed to load notifications count", err));
    }
  }, [user]);
          ...
          >
            <div className="flex items-center gap-4">
              <item.icon 
                ...
                className={isActive ? "text-[var(--sc-brand-600)]" : "text-gray-400 group-hover:text-[var(--sc-text-primary)]"} 
              />
            ...
            {displayBadge > 0 && (
              <span className={`flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold ${isActive ? 'bg-[var(--sc-brand-100)] text-[var(--sc-brand-600)]' : 'bg-gray-100 text-gray-500'}`}>
                {displayBadge}
              </span>
            )}
          </NavLink>
