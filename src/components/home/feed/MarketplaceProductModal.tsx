...
export default function MarketplaceProductModal({ item, isOpen, onClose }: MarketplaceProductModalProps) {
          ...
          <div className="flex flex-col md:flex-row gap-6 md:gap-10">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-3">
                <span className="bg-[var(--sc-brand-50)] text-[var(--sc-brand-700)] text-[11px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">
                  {item.category}
                </span>
                {item.isService && (
                  <span className="bg-[var(--sc-brand-500)] text-white text-[11px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">
                    Service
                  </span>
                )}
              </div>
              ...
              <p className="text-[14px] text-gray-500 mb-6 font-medium">
                Sold by <span className="text-[var(--sc-brand-600)] cursor-pointer">{item.seller}</span>
              </p>

              <h3 className="font-bold text-[16px] text-[var(--sc-text-primary)] mb-2">Description</h3>
              <p className="text-[15px] text-gray-500 leading-relaxed mb-6">
                {item.description}
              </p>

              <h3 className="font-bold text-[16px] text-[var(--sc-text-primary)] mb-3">Features</h3>
              <ul className="flex flex-col gap-2">
                {item.features.map((feature, idx) => (
                  <li key={idx} className="flex items-center gap-2 text-[14px] text-gray-600 bg-gray-50 px-4 py-2.5 rounded-xl border border-[var(--sc-border)]">
              ...
              <div className="bg-gray-50 rounded-2xl p-5 border border-[var(--sc-border)]">
                ...
                {item.isService ? (
                  <div className="flex flex-col gap-3 mb-6 border-t border-[var(--sc-border)] pt-4">
                    <label className="text-[13px] font-bold text-[var(--sc-text-secondary)]">Select Date</label>
                    <input type="date" className="w-full bg-white border border-[var(--sc-border)] px-3 py-2 rounded-xl focus:outline-none focus:border-[var(--sc-brand-500)] text-[14px]" />
                    <label className="text-[13px] font-bold text-[var(--sc-text-secondary)]">Select Time</label>
                    <input type="time" className="w-full bg-white border border-[var(--sc-border)] px-3 py-2 rounded-xl focus:outline-none focus:border-[var(--sc-brand-500)] text-[14px]" />
                  </div>
                ) : (
                  <div className="flex flex-col gap-2 mb-6 border-t border-[var(--sc-border)] pt-4">
                    <div className="flex justify-between text-[13px] text-gray-500">
                      <span>Delivery:</span>
                      <span className="font-bold text-[var(--sc-text-primary)]">2-3 Days</span>
                    </div>
                    <div className="flex justify-between text-[13px] text-gray-500">
                      <span>Shipping:</span>
                      <span className="font-bold text-[var(--sc-text-primary)]">৳60</span>
                    </div>
                  </div>
                )}
                ...
                >
                  {item.inStock ? (item.isService ? 'Confirm Booking' : 'Add to Cart') : 'Out of Stock'}
                </button>
              </div>
            </div>
