    ...
    <div className="mt-2.5">
        ...
        {mediaList.slice(0, 4).map((m, i) => (
          <div
            ...
            className={`relative cursor-pointer bg-black ${
              mediaList.length === 1
                ? "col-span-2 aspect-video sm:h-80"
                : mediaList.length === 3 && i === 0
                  ? "row-span-2 min-h-[160px]"
      ...
      {activeMedia !== null && (
        ...
        >
          <button
            onClick={() => setActiveMedia(null)}
            className="absolute top-4 right-4 text-white bg-white/20 p-2 rounded-full hover:bg-white/30 z-20"
          >
            <svg
              width="20"
              height="20"
          ...
          <button
            ...
            className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 text-white bg-white/20 p-2.5 sm:p-3 rounded-full hover:bg-white/30 z-20 flex"
          >
            ‹
          </button>

          <div
            className="max-w-3xl w-full max-h-[85vh] flex flex-col items-center px-2"
          ...
          >
            {mediaList[activeMedia].type === "video" ? (
              <video
                ...
                controls
                autoPlay
                className="max-w-full max-h-[65vh] sm:max-h-[70vh] rounded-xl object-contain"
              />
            ) : (
              <img
                src={mediaList[activeMedia].url}
                alt={`media ${activeMedia}`}
                className="max-w-full max-h-[65vh] sm:max-h-[70vh] object-contain rounded-xl"
              />
            )}
            <div className="flex gap-2 mt-3 overflow-x-auto max-w-full pb-1">
              {mediaList.map((m, i) => (
                <button
                  key={i}
                  onClick={() => setActiveMedia(i)}
                  className={`w-12 h-12 sm:w-16 sm:h-16 rounded-lg overflow-hidden border-2 shrink-0 ${i === activeMedia ? "border-white" : "border-transparent opacity-60"}`}
                >
                  {m.type === "video" ? (
                    <video
                      src={m.url}
          ...
          <button
            ...
            className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 text-white bg-white/20 p-2.5 sm:p-3 rounded-full hover:bg-white/30 z-20 flex"
          >
            ›
          </button>
        </div>
