...
const categoryConfig: Record<string, { label: string; colorClass: string }> = {
  ...
  fundraise: {
    label: "Fundraise",
    colorClass: "bg-[var(--sc-brand-100)] text-[var(--sc-brand-700)]",
  },
};
...
export default function PostCard({
  ...
  const displayAuthorName =
    isOwnPost && user?.displayName ? user.displayName : authorName || "User";
  const displayAvatar =
    isOwnPost && user?.photoUrl ? user?.photoUrl : authorAvatar;
  const rawHandle =
    isOwnPost && (user as any)?.handle
      ? (user as any).handle
      : authorHandle ||
        displayAuthorName.toLowerCase().replace(/[^a-z0-9_]/g, "") ||
        "user";
  ...
  useEffect(() => {
    if (user) {
      fetchLikeStatus(id, user.uid)
        .then((res) => setIsLiked(res.liked))
        .catch(console.error);
      fetchBookmarkStatus(id, user.uid)
  ...
  return (
    <article
      className={`bg-white rounded-2xl p-3.5 sm:p-5 mb-3 sm:mb-4 border border-[var(--sc-border)] overflow-hidden w-full max-w-full box-border shadow-xs transition-all duration-300 ease-out ${
        isVanishing
          ? "opacity-0 scale-90 -translate-y-4 max-h-0 !p-0 !mb-0 !border-0 pointer-events-none"
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
          ...
          >
            <div
              className={`w-10 h-10 rounded-full overflow-hidden flex items-center justify-center bg-gray-100 shrink-0 ${isVerified ? "ring-2 ring-offset-2 ring-[var(--sc-brand-500)]" : ""}`}
            >
              {displayAvatar ? (
                <img
                  src={displayAvatar}
          ...
          <div className="flex flex-col min-w-0 flex-1 justify-center">
            <div className="flex items-center gap-1.5 min-w-0">
              <Link
                to={`/profile?id=${authorId}`}
                className="font-bold text-[14px] sm:text-[15px] text-[var(--sc-text-primary)] hover:underline truncate notranslate"
                translate="no"
              >
                {displayAuthorName}
              </Link>
              <span
                className={`px-2 py-0.5 rounded-full text-[9px] font-bold shrink-0 ${cat.colorClass}`}
              >
                {cat.label}
              </span>
              {media?.[0]?.hyperidTag && (
            ...
            <div className="flex items-center gap-1 text-[11px] sm:text-[12px] text-[var(--sc-text-muted)] truncate mt-0.5">
              <span
                className="text-gray-500 font-medium shrink-0 notranslate"
                translate="no"
              >
                {displayHandle}
              </span>
      ...
      {isEditing ? (
        <div className="mb-4">
          ...
          <div className="flex justify-end gap-2 mt-2">
            ...
            <button
              ...
              className="text-sm bg-[var(--sc-brand-500)] text-white px-4 py-1.5 rounded-full font-bold"
            >
              Save
            </button>
          </div>
        ...
        <p className="text-[var(--sc-text-primary)] text-[14px] sm:text-[15px] mb-3.5 whitespace-pre-wrap break-words leading-relaxed">
          {editContent}
        </p>
      )}
      ...
      {category === "fundraise" && fundraiseGoal && (
        <div className="mb-3.5 bg-[var(--sc-brand-50)] rounded-xl p-3.5 sm:p-4 border border-[var(--sc-brand-100)] w-full box-border">
          <div className="flex justify-between text-[12px] sm:text-[13px] mb-2 font-bold">
            <span className="text-[var(--sc-brand-800)] truncate mr-2">
              Raised:{" "}
              <span className="notranslate" translate="no">
                ৳{raisedAmount.toLocaleString()}
              </span>
            </span>
            <span className="text-[var(--sc-brand-600)] shrink-0">
              Goal:{" "}
              <span className="notranslate" translate="no">
                ৳{fundraiseGoal.toLocaleString()}
              </span>
            </span>
          </div>
          <div className="w-full bg-[var(--sc-brand-200)] rounded-full h-2">
            <div
              className="bg-[var(--sc-brand-600)] h-2 rounded-full transition-all duration-500"
              style={{
                width: `${Math.min((raisedAmount / fundraiseGoal) * 100, 100)}%`,
              }}
            ></div>
          </div>
          <div className="text-[11px] sm:text-[12px] text-[var(--sc-brand-600)] mt-2 flex justify-between items-center">
            ...
            <span className="font-medium">
              {Math.min(100, Math.round((raisedAmount / fundraiseGoal) * 100))}%
              funded
            </span>
          </div>
          <button
