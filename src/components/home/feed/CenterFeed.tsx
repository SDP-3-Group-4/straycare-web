...
export default function CenterFeed() {
  ...
  const fetchPostsWithLocation = (lat?: number, lng?: number) => {
    ...
    fetchPosts(
      activeTab,
      activeTab === "nearby" ? user?.uid : undefined,
      lat,
      lng,
    )
      .then((data) => {
        ...
        if (Array.isArray(data)) {
          ...
          console.log(
            "[CenterFeed] Setting sorted posts count:",
            sorted.length,
          );
          setPosts(sorted);
        } else {
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-3">
            {[1, 2].map((i) => (
              <div
                key={i}
                className="bg-white rounded-2xl p-4 sm:p-5 border border-[var(--sc-border)] animate-pulse shadow-xs"
              >
            ...
            <div className="text-center py-2 text-xs text-gray-400 flex items-center justify-center gap-1.5">
              <Wifi
                size={13}
                className="text-[var(--sc-brand-600)] animate-pulse"
              />
          ...
          <div className="p-12 text-center text-gray-500 bg-white rounded-2xl border border-[var(--sc-border)] shadow-xs">
            <p className="font-semibold text-[15px] text-gray-700 mb-1">
              No posts found
            </p>
            <p className="text-xs text-gray-400">
              Be the first to share an update or rescue story!
            </p>
          </div>
        ) : (
          posts.map((post) => (
