...
export default function MarketItemCard({ item, onClick, onAdd }: MarketItemCardProps) {
  return (
    <div 
    ...
    >
      ...
      <div className="relative w-full aspect-[4/3] bg-gray-50 border-b border-[var(--sc-border)] overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-2">
          <span className="bg-white/90 backdrop-blur-md text-[var(--sc-brand-700)] text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider border border-[var(--sc-brand-100)]">
            {item.category}
          </span>
          {item.isService && (
            <span className="bg-[var(--sc-brand-500)] text-white text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider w-fit">
              Service
            </span>
          )}
        </div>
      ...
      <div className="p-5 flex flex-col flex-1 bg-white relative">
        ...
        <div className="flex items-center gap-1.5 mb-5 mt-auto">
          ...
          <span className="font-bold text-[13px] text-[var(--sc-text-primary)]">{item.rating}</span>
