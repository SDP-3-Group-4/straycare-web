...
export default function ProfileHeader({
  ...
  useEffect(() => {
    if (authUser && user.id && !isOwnProfile) {
      fetchConnectionStatus(authUser.uid, user.id)
        .then((data) => {
          setConnectionStatus(data.status);
        })
        .catch(console.error);

      fetchMutualConnections(authUser.uid, user.id)
        .then((data) => {
          if (data) setMutualsData(data);
        })
        .catch(console.error);

      fetchGraphDegree(authUser.uid, user.id)
        .then((deg) => {
  ...
  return (
    <div className="bg-white border border-[var(--sc-border)] rounded-2xl overflow-hidden mb-4 sm:mb-6 shadow-xs w-full box-border">
      ...
      <div className="h-36 sm:h-48 w-full bg-slate-200 relative group overflow-hidden">
        ...
        {isOwnProfile && (
          <label
            className="absolute top-3 right-3 bg-white/90 hover:bg-white text-gray-800 p-2 rounded-full cursor-pointer shadow-md opacity-90 sm:opacity-0 group-hover:opacity-100 transition-opacity z-10"
            title="Edit cover"
          >
            {uploadingCover ? (
      ...
      <div className="px-4 sm:px-6 pb-5 sm:pb-6 relative">
        ...
        <div className="flex justify-between items-end -mt-10 sm:-mt-12 mb-3 sm:mb-4 gap-2">
          <div className="relative group shrink-0">
            ...
            {isOwnProfile && (
              <label
                className="absolute bottom-0 right-0 bg-white border border-gray-200 text-gray-700 p-1.5 rounded-full cursor-pointer shadow-xs hover:bg-gray-50 transition-colors z-10"
                title="Edit avatar"
              >
                {uploadingAvatar ? (
        ...
        <div className="mb-3">
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
            <h1
              className="text-xl sm:text-2xl font-black text-[var(--sc-text-primary)] notranslate"
              translate="no"
            >
              {user.name}
            </h1>
            {user.isVerified && (
              <span className="flex items-center gap-1 px-2 py-0.5 bg-[var(--sc-brand-50)] text-[var(--sc-brand-600)] text-[10px] sm:text-[11px] font-bold rounded-lg flex-shrink-0">
                <BadgeCheck size={13} />
                Verified Vet
              </span>
            )}
            {user.isVet && (
              <span className="flex items-center gap-1 px-2 py-0.5 bg-green-100 text-green-700 text-[10px] sm:text-[11px] font-bold rounded-lg flex-shrink-0">
                <ShieldCheck size={13} />
                Verified Vet
              </span>
            )}
            {user.topContributor && (
              <span className="flex items-center gap-1 px-2 py-0.5 bg-[var(--sc-brand-100)] text-[var(--sc-brand-700)] text-[10px] sm:text-[11px] font-bold rounded-lg flex-shrink-0">
                ⭐ Top Contributor
              </span>
            )}
          </div>
          <div className="flex items-center gap-2">
            <p
              className="text-[var(--sc-text-secondary)] font-medium text-xs sm:text-sm notranslate"
              translate="no"
            >
        ...
        {user.pets && user.pets.length > 0 && (
          <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-3">
            {user.pets.map((pet, idx) => (
              <span
                key={idx}
                className="flex items-center gap-1 px-2.5 py-0.5 bg-orange-50 border border-orange-100 text-orange-700 text-[12px] sm:text-[13px] font-bold rounded-full notranslate"
                translate="no"
              >
          ...
          {user.website && (
            <div className="flex items-center gap-1">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-[var(--sc-brand-600)] hover:underline truncate max-w-[180px]"
              >
                {user.website}
              </a>
            </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mt-4 pt-3 border-t border-gray-100">
          <button
            ...
            className="flex items-center gap-1.5 hover:underline text-left disabled:cursor-default"
            disabled={!onConnectionsClick}
          >
            <span className="font-bold text-[var(--sc-text-primary)] text-sm sm:text-base">
              {connectionsCount}
            </span>
          ...
          {!isOwnProfile && mutualsData.count > 0 && (
            <div className="flex items-center gap-2 bg-blue-50/70 border border-blue-100 px-3 py-1.5 rounded-xl">
              ...
              <span className="text-[12px] font-semibold text-blue-900">
                {mutualsData.count} mutual connection
                {mutualsData.count > 1 ? "s" : ""}
                {mutualsData.mutuals[0]
                  ? ` including ${mutualsData.mutuals[0].displayName}`
                  : ""}
              </span>
            </div>
          )}
        ...
        <div className="mt-4 rounded-2xl border border-[var(--sc-brand-100)] bg-[var(--sc-brand-50)]/40 p-3.5 sm:p-4">
          <div className="flex items-center justify-between mb-2.5">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[13px] sm:text-[14px] font-bold text-[var(--sc-text-primary)]">
          ...
          {totalGoal > 0 ? (
            <>
              ...
              <div className="h-2 rounded-full bg-gray-200 overflow-hidden mb-2.5">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, Math.round((totalRaised / totalGoal) * 100))}%`,
                  }}
                />
              </div>
            </>
            ...
            <p className="text-[12px] text-gray-500 mb-2">
              No active fundraisers created yet.
            </p>
          )}
