...
export default function ChatInfoModal({ isOpen, onClose, onDeleted, chat }: ChatInfoModalProps) {
  const [isMuted, setIsMuted] = useState(false);
  const [isEditingName, setIsEditingName] = useState(false);
  const [groupName, setGroupName] = useState(chat.name);
  const [confirmAction, setConfirmAction] = useState<null | 'delete' | 'clear' | 'leave' | 'block'>(null);
  ...
  const toggleMute = () => {
    const muted = JSON.parse(localStorage.getItem('sc_muted_chats') || '{}');
    const next = !isMuted;
    if (next) muted[chat.id] = true; else delete muted[chat.id];
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      if (confirmAction === 'delete' || confirmAction === 'leave') await deleteChat(id);
      ...
      <div className="relative bg-white rounded-3xl w-full max-w-sm max-h-[90vh] overflow-hidden flex flex-col border border-[var(--sc-border)]">
        ...
        <div className="overflow-y-auto pb-6">
          ...
          <div className="flex flex-col p-3">
            ...
            <button onClick={() => setConfirmAction('clear')} className="flex items-center gap-4 p-3 rounded-xl hover:bg-gray-50 transition-colors w-full text-left">
              <div className="w-10 h-10 rounded-full bg-gray-100 text-gray-600 flex items-center justify-center">
                <MessageSquareOff size={18} />
              </div>
              <div className="flex flex-col flex-1">
                <span className="font-bold text-[15px] text-[var(--sc-text-primary)]">Clear Chat</span>
                <span className="text-[12px] text-gray-500">Remove all messages</span>
              </div>
            </button>
            ...
            {chat.isGroup && (
              <>
                ...
                <button onClick={() => alert('Manage members coming soon')} className="flex items-center gap-4 p-3 rounded-xl hover:bg-gray-50 transition-colors w-full text-left">
                ...
                <button onClick={() => setConfirmAction('leave')} className="flex items-center gap-4 p-3 rounded-xl hover:bg-red-50 transition-colors w-full text-left group">
            ...
            {!chat.isGroup && !chat.isAiBot && (
              <>
                <div className="h-px bg-[var(--sc-border)] my-2 mx-3"></div>
                <button onClick={() => setConfirmAction('block')} className="flex items-center gap-4 p-3 rounded-xl hover:bg-red-50 transition-colors w-full text-left group">
                  <div className="w-10 h-10 rounded-full bg-red-50 text-red-600 group-hover:bg-red-100 flex items-center justify-center transition-colors">
                    <ShieldAlert size={18} />
                  </div>
            ...
            <button onClick={() => setConfirmAction('delete')} className="flex items-center gap-4 p-3 rounded-xl hover:bg-red-50 transition-colors w-full text-left group mt-1">
              <div className="w-10 h-10 rounded-full bg-red-50 text-red-600 group-hover:bg-red-100 flex items-center justify-center transition-colors">
                <Trash2 size={18} />
              </div>
        ...
        {confirmAction && (
          <div className="absolute inset-0 bg-white/95 backdrop-blur-sm flex flex-col items-center justify-center p-6 text-center">
            <div className={`w-14 h-14 rounded-full flex items-center justify-center mb-4 ${confirmAction === 'delete' || confirmAction === 'block' ? 'bg-red-100 text-red-600' : 'bg-amber-100 text-amber-600'}`}>
              {confirmAction === 'delete' ? <Trash size={24} /> : confirmAction === 'block' ? <ShieldAlert size={24} /> : confirmAction === 'leave' ? <LogOut size={24} /> : <Trash2 size={24} />}
            </div>
            <h3 className="font-bold text-[16px] text-[var(--sc-text-primary)]">
              {confirmAction === 'delete' ? 'Delete chat?' : confirmAction === 'clear' ? 'Clear messages?' : confirmAction === 'leave' ? 'Leave group?' : 'Block user?'}
            </h3>
            <p className="text-[13px] text-gray-500 mt-1 max-w-[240px]">
              {confirmAction === 'delete' ? `This will permanently delete your conversation with ${chat.name}.` : confirmAction === 'clear' ? 'All messages will be removed but the chat will remain.' : confirmAction === 'leave' ? 'You will be removed from the group.' : `You won’t receive messages from ${chat.name} again.`}
            </p>
            <div className="flex gap-3 mt-6 w-full">
              <button disabled={isProcessing} onClick={() => setConfirmAction(null)} className="flex-1 py-2.5 rounded-xl border border-[var(--sc-border)] font-bold text-[14px] disabled:opacity-50">Cancel</button>
              <button disabled={isProcessing} onClick={handleAction} className={`flex-1 py-2.5 rounded-xl font-bold text-[14px] text-white disabled:opacity-50 ${confirmAction === 'delete' || confirmAction === 'block' ? 'bg-red-600 hover:bg-red-700' : 'bg-[var(--sc-brand-600)] hover:bg-[var(--sc-brand-700)]'}`}>{isProcessing ? '...' : 'Confirm'}</button>
            </div>
          </div>
        )}
      </div>
