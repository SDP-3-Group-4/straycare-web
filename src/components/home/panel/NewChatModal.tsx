...
import { X, Search, Users, Check, Loader2, UserPlus, ShieldAlert } from 'lucide-react';
      ...
      fetchConnections(user.uid)
        .then((data: any[]) => {
          const connectedUsers = data
            ...
            .map((c: any) => (c.requesterId === user.uid ? c.recipient : c.requester))
            .filter((u: any) => u && u.id !== user.uid && u.id !== 'ai-vet-bot-id');
  ...
  const toggleUser = async (id: string) => {
    if (mode === 'direct') {
      try {
        ...
        alert(err?.message || "Chats are only available between connected users.");
      } finally {
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          ...
          <div className="p-4 border-b border-[var(--sc-border)] shrink-0 bg-gray-50/50 dark:bg-white/[0.02]">
            ...
            <div className="relative group">
              <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[var(--sc-brand-500)] transition-colors" size={17} />
              <input 
                type="text" 
          ...
          <div className="flex-1 overflow-y-auto p-2.5">
            {mode === 'group' && selectedUsers.length > 0 && (
              <div className="px-2 py-1.5 flex flex-wrap gap-1.5 mb-2 border-b border-[var(--sc-border)] pb-2.5">
                {selectedUsers.map((id) => {
                  ...
                  return u ? (
                    <div key={id} className="flex items-center gap-1.5 bg-[var(--sc-brand-50)] dark:bg-purple-950/40 border border-[var(--sc-brand-200)] dark:border-purple-800 rounded-full pl-1 pr-2 py-0.5">
                      <img src={u.photoUrl} alt={u.displayName} onError={avatarOnError} className="w-5 h-5 rounded-full object-cover" />
                      <span className="text-[11px] font-bold text-[var(--sc-brand-700)] dark:text-purple-300 notranslate" translate="no">
                        {u.displayName.split(' ')[0]}
                      </span>
                      <button onClick={() => toggleUser(id)} className="text-[var(--sc-brand-400)] hover:text-[var(--sc-brand-600)] ml-0.5">
            ...
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-12 text-gray-400 gap-2">
                <Loader2 size={24} className="animate-spin text-[var(--sc-brand-500)]" />
                <span className="text-xs font-medium">Loading connections...</span>
              </div>
            ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="relative shrink-0">
                        <img src={u.photoUrl} alt={u.displayName} onError={avatarOnError} className="w-10 h-10 rounded-full object-cover border border-[var(--sc-border)]" />
                        {mode === 'group' && (
                          <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full flex items-center justify-center border border-white transition-colors ${
                            isSelected ? 'bg-[var(--sc-brand-600)] text-white' : 'bg-gray-200'
                          }`}>
                      ...
                      <div className="flex flex-col flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          <span className="font-bold text-[14px] text-[var(--sc-text-primary)] truncate notranslate" translate="no">
                            {u.displayName}
                          </span>
                        ...
                        <span className="text-[12px] text-gray-400 truncate notranslate" translate="no">
                          {formatHandle(u.handle)}
                        </span>
                      </div>
                      {isStarting && (
                        <Loader2 size={16} className="animate-spin text-[var(--sc-brand-600)] shrink-0 ml-2" />
                      )}
                    </button>
                  );
                })}
                ...
                {!isLoading && users.length === 0 && (
                  <div className="p-6 text-center flex flex-col items-center justify-center my-4">
                    ...
                    <p className="text-xs text-[var(--sc-text-secondary)] mt-1.5 leading-relaxed max-w-[260px] mx-auto">
                      Direct messaging is exclusively available between connected users. Send connection requests to pet lovers or doctors to chat!
                    </p>
                    <button
                      onClick={() => {
                        onClose();
