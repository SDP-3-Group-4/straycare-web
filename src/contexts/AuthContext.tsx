...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const syncWithPostgres = async (
    ...
    try {
      ...
      if (!dbUser) {
        const displayName =
          initialData.displayName || initialData.email?.split("@")[0] || "User";
      ...
      const appUser: AppUser = {
        ...
        emailVerified:
          initialData.emailVerified || dbUser.emailVerified || false,
  ...
  const refreshFromFirebase = useCallback(
    async (
      fbUser: FirebaseUser,
      extra?: { phone?: string; referralCode?: string },
    ) => {
      ...
      try {
        ...
        if (currentUser) {
          setUser({
            ...
            photoUrl:
              currentUser.photoURL || (currentUser as any).photoUrl || null,
  ...
  const signUp = async ({
    ...
    await syncWithPostgres(
      credential.user.uid,
      {
        ...
        displayName,
        photoUrl:
          credential.user.photoURL || (credential.user as any).photoUrl || null,
        emailVerified: false,
      },
      { phone, referralCode },
    );
